...
class Program
{
    public async static Task Main(string[] args)
    {
        ...
        await SyncNetworkElements(dbContext);

        await Run(serviceProvider);
        await InsertCuttingDown(dbContext);
        while (true)
        {
    ...
    private static async Task ResetDatabase(MyDbContext dbContext)
{
    try
    {
    ...
    private static async Task SyncNetworkElements(MyDbContext dbContext)
    {
        try
        {
    ...
    private static async Task InsertCuttingDown(MyDbContext dbContext)
    {
        try
        {
...
private static async Task SeedData(MyDbContext dbContext)
{
    var faker = new Faker();
    ...
    if (!dbContext.NetworkElementHierarchyPaths.Any())
    {
        hierarchyPaths = new[]
        {
            new NetworkElementHierarchyPath { NetwrokElementHierarchyPathName = "Governrate, Sector, Zone, City, Station, Tower, Cabin, Cable, Buidling, Flat, Individual Subscription" },
            new NetworkElementHierarchyPath { NetwrokElementHierarchyPathName = "Governrate, Sector, Zone, City, Station, Tower, Cabin, Cable, Buidling, Corporate Subscription" }
        }.ToList();
